...
function setup() {
    ...
    octopi = Array(ARRAY_SIZE)
        ...
        .map(row => row.map((_) => map(random(), 0, 1, 0, MAX_ENERGY)));
}
function draw() {
    console.log('draw');
...
const getNeighborCoordinates = ([x, y], array) => {
 ...
 */
const triggerFlashpoints = (octopi, flashpoints) => {
...
const zeroOutNegatives = (array) => array.map(row => row.map(cell => (cell < 0 ? 0 : cell)));
const step = (octopi) => {
